...
const num = 123.567;
// const totla = Math.abc(num);
//const total = Math.max(fruiDB[0].price, fruiDB[1], price, fruiDB[2].price);
//console.log(total);
...
bodyDOM.style.backgroundImage = `url(./images/bg_0${Math.floor(Math.random()*5)+1}.jpg)`

// 생년월일 입력 후'계산' 버튼 클릭시 생일부터 지금까지 몇일이 지났는지 , 몇시간이 지났는지 보여주는 프로그램(양력기준)
//지금까지 흐른 날짜 게산법 : 1000*60*60*24로 나누기
//지금까지 흐른 시간 게산법 : 1000*60*60로 나누기
...
function birthFunc(){
    ...
    //오늘날짜에서 생년월일 빼기
    //new Date 복잡한 특수문자 및 문자를 포함하기 때문에 숫자만 변환해서 계산하도록 getTime() 함수 활용
