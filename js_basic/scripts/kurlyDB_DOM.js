...
// 무산 스크롤로 인해서 불러오는 DB 값이 매번 다르므로 필요한 변수 생성
// 한번에 출력되는 개수는 4
const itemLoad = 4;
...
loadMoreContent() // 처음보이는 상품을 위해서 함수를 바로 호출하는것
function loadMoreContent(){
    // const newLi = document.createElement('li');
    //이미지 -> 제품명 -> 요약설명 -> 가격 -> 할인가 -> 세일 -> 리뷰
    // Math.round((원가 * (1 - 할인율 / 100))).toLocaleString('ko-kr') // 소수점반올림
    ...
    for(let i=startIndex;i<endIndex;i++){
        ...
        newLi.innerHTML += `<p class="price">${Math.round((kurlyDB[i].price * (1 - kurlyDB[i].sale / 100))).toLocaleString('ko-kr')} 할인가임</p>`
        newLi.innerHTML += `<p class="sale">${kurlyDB[i].sale} %</p>`
        newLi.innerHTML += `<p class="review">${kurlyDB[i].review}+</p>`
        container.appendChild(newLi);
        console.log(newLi);
    }
    // 위 for문 한번 출력 후 스크롤 이벤트로 인해 재출력 시 endIndex 값이 다음 출력되는 DB 개수만큼 증가되게 수정
    endIndex += itemLoad;
    isLoading = false;
}
...
window.addEventListener('scroll',()=>{
    ...
    // if(adTop < window.innerHeight * 0.5){
    ...
    const scrollYvalue = 0.8;//0~1 기준으로 사용자의 스크롤 위치 파악값
    //console.log(`현재 뷰포트값 ${currentScroll}, 전체높이 ${totalHeight}`)

    // 현재 스크롤 위치가 전체 스크롤 위치대비 80% 뷰포트 위치에 왔는지 + DB 로드 안된 상태인지
    // 현재 뷰포트 값 >= 전체스크롤값 * 사용자위치값
    if(currentScroll >= totalHeight * scrollYvalue &&! isLoading){
        // 로드된 종료 인덱스가 DB 전체 개수와 같으면 종료
        if(endIndex >= kurlyDB.length){return}
        console.log('새로운 컨텐츠 로드중')
        isLoading = true;
        loadMoreContent();
    }
