// DOM EVENT
// HTML 태그에서 JS로 동적제어하고 싶은 대상만 변수로 만든다.
// 회원만 주문되는 주문하기 버튼 알고리즘
// (비회원인 경우) 주문하기 버튼을 클릭했을 때 '회원만 주문할 수 있습니다.'메세지 출력
// 주문하기 버튼 == DOM 개겣변수
// 클릭했을 때 == 이벤트
// 회원만 메세지 출력 == 함수
// 이벤트리스너 -> 리스너(listener)란>  특정 이벤트가 발생했을대 실행할 함수를 의미한다.
const buyBtn = document.querySelector('#buyBtn');
console.log(buyBtn); //변수정상확인

//버튼 onclick 이벤트리스터로 호출할 함수 생성
function userOrderMsg(){
    return console.log('회원만 주문 가능합니다. 로그인해주세요.')
}
// 결제하기 클릭 시 결제완료되었습니다 출력 알고리즘 
const payBtn = document.querySelector('#payBtn');
console.log(payBtn)
function userPayMsg(){
    return console.log('결제가 완료되었습니다.')
}
// 아이디 중복확인 클릭 시 아이디가 중복되지않습니다 출력 알고리즘
const idBtn = document.querySelector('#idBtn');
console.log(idBtn);
function userId(){
    window.alert('아 쓰던가;;')
    return console.log('아이디가 중복되지 않습니다. 사용하셔도 좋습니다.')
}
console.log('=============================================키보드이벤트')
//키보드 이벤트
// 리뷰입력창에 키보드를 눌렀을때 '50자 이상 입력하세요' 출력 알고리즘 
const userReview = document.querySelector('input[name*=review]');
console.log(userReview);
function revuew50Msg(){
    return console.log('50자 이상 입력하세요')
}
// id 입력창 객체 준비 후 키보드를 아무키나 입력했을때 '아이디는 영어로 시작해야합니다' 출력 알고리즘
const user50Id = document.querySelector('input[name*=id]');
console.log(user50Id)
function userIfId(){
    return console.log('아이디는 영어로 시작해야합니다')
}

// console.log(userId);
// pw 입력창 객체 준비 후 키보드를 아무키나 입력했을 때 '비밀번호는 특수문자 포함 12글자 이상 입력하세요' 출력 알고리즘 
const user50pw = document.querySelector('input[name*=pw]');
console.log(user50pw)
function userIfPw(){
    return console.log('비밀번호는 특수문자 포함 12글자 이상 입력하세요')
}
//기타이벤트
//focus 특정 요소에 포커스(커서 활성화)됐을 경우
user50Id.style.border = 'none'; //즉시 실행 테스트
function focusblue(){
    user50Id.style.border = '3px solid blue';
}
function blurblue(){
    user50Id.style.border = '3px solid red';
}
// 쇼핑몰 상품 수량 입력 알고리즘
// 기본 수량 1
// 사용자가 숫자 입력을 위해서 커서를 두면 배경색이 노랑변경
const itemNum = document.querySelector('input[name*=num]')
console.log(itemNum);
// 커서를 빼면 배경색 흰색
function focusNum(){
    return itemNum.style = 'background-color : yellow';
}
function blurNum(){
    return itemNum.style = 'background-color : white';
}
//문서가 로딩되었을때 실행함수
// function loadMsg(){
//     return alert('문서 로딩완료')
// }
//=================================
console.log('============================================이벤트와 콜백함수')
const googleJoin = document.querySelector('.google_join');
console.log(googleJoin);
// 자바스크립트 작성하는 객체 이벤트 문법
// 객체.addEventListener('이벤트',콜백함수);
// addEventListener 이벤트 작성 시 'on'없이 이벤트만 작성한다.
googleJoin.addEventListener('click',googleGo);
function googleGo(){
    return window.open('http://google.com','_blank')
}

const naverJoin = document.querySelector('.naver_join')
const kakaoJoin = document.querySelector('.kakao_join')
console.log(naverJoin)
console.log(kakaoJoin)

naverJoin.addEventListener('click',naverGO)
kakaoJoin.addEventListener('click',kakaoGO)

function naverGO(){
    return window.open('http://naver.com','_blank')
}
function kakaoGO(){
    return window.open('http://kakao.com','_blank')
}
// 현재 페이지 변경밥업
// 1. 객체
const fileLink = document.querySelector('.file_link')
console.log(fileLink);
// 2.이벤트
fileLink.addEventListener('click',link0530);
//3. 함수
//JS에서 상대경로 연결 시 JS 파일 기준이 아닌 JS가 연결되 ㄴ HTML파일이 저장된 이치를 기준으로 상대경로를 결정한다.
function link0530(){
    return window.location.href = 'bom1.html';
}
//index, readme 경로만들기
const indexLink = document.querySelector('.index_link');
const readmeLink = document.querySelector('.readme_link');
console.log(indexLink);
console.log(readmeLink);
indexLink.addEventListener('click',indexGoLink)
readmeLink.addEventListener('click',readmeGoLink)

function indexGoLink(){
    return window.location.href = 'index.html';
}
function readmeGoLink(){
    return window.location.href = '../README.md';
}
//상품 수량증가 알고리즘
//1. 수량 초기값 1
//2. + 버튼 클릭 시 수량이 1씩 증가해야한다.
const num = document.querySelector('form input');
const numPlus = document.querySelector('#num_btn');
const numMinus = document.querySelector('#num_btn_minus')
const numReset = document.querySelector('#num_btn_reset')
console.log(num);
console.log(numPlus);
console.log(numMinus);
console.log(numReset);

num.value = 1; //1. 수량 초기값 1

//2. 버튼 클릭시
numPlus.addEventListener('click',plusBtn)
numMinus.addEventListener('click',minusBtn)
numReset.addEventListener('click',resetBtn)

// 더하는 함수
function plusBtn(){
    //수량이 1씩 증가한다.
    //1. 증감연산자 사용법
    return num.value = ++num.value;
    //2. 더하기 연산자와 형변환메서드 활용법
    //return num.value = Number(num.value)+1; 
}
//뺴는 함수
function minusBtn(){
    return num.value = --num.value;
}
//리셋하는 함수
function resetBtn(){
    return num.value = 1;
}
